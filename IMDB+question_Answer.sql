...
-- Type your code below:
SELECT 
    SUM(ISNULL(id)) AS ID_nulls,
    SUM(ISNULL(title)) AS title_nulls,
    SUM(ISNULL(year)) AS year_nulls,
    SUM(ISNULL(date_published)) AS date_published_nulls,
    SUM(ISNULL(duration)) AS duration_nulls,
    SUM(ISNULL(country)) AS country_nulls,
    SUM(ISNULL(worlwide_gross_income)) AS worlwide_gross_income_nulls,
    SUM(ISNULL(languages)) AS languages_nulls,
    SUM(ISNULL(production_company)) AS production_company_nulls
FROM
    movie;
...
-- Number of movies released in year
SELECT 
...
-- Number of movies released in month
SELECT 
...
-- Type your code below:
SELECT 
    genre, COUNT(movie_id) AS num_of_movies
FROM
    genre g
        INNER JOIN
...
SELECT 
	m.production_company, COUNT(m.id) AS movie_count,
...
(
SELECT 
    m.id, m.date_published, r.median_rating
FROM
    movie m
        INNER JOIN
...
SELECT 
    ...
    SUM(ISNULL(known_for_movies)) AS known_for_movies_null
FROM
    names;
...
-- Type your code below:
SELECT 
	n.name as actor_name,r.total_votes, 
...
SELECT 
    ...
    r.total_votes,COUNT(rm.movie_id) AS movie_count,
	ROUND(SUM(avg_rating*total_votes)/SUM(total_votes),2) AS actor_avg_rating,
RANK() OVER(ORDER BY ROUND(SUM(avg_rating*total_votes)/SUM(total_votes),2)DESC) AS actoress_avg_rating
FROM 
	names n 
	INNER JOIN role_mapping rm
ON 
...
--------------------------------------------------------------------------------------------*/
-- Type your code below:
WITH thrill_movie AS
(
SELECT DISTINCT
    m.title, g.genre, r.avg_rating
FROM
    genre g
        INNER JOIN
